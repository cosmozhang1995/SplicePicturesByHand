...
QImage * BackgroundDodging::dodgeBackground(QImage &image, QImage &background) {
    ...
    QImage *processedImage = new QImage(imageWidth, imageHeight, QImage::Format_RGB888);
...
//            int r = qRed(rgbIm) - qRed(rgbBg);
//            int g = qGreen(rgbIm) - qGreen(rgbBg);
//            int b = qBlue(rgbIm) - qBlue(rgbBg);
//            processedImage->setPixel(col, row, qRgb(r, g, b));
    ...
    for (int row = 0; row < imageHeight; row++) {
        for (int col = 0; col < imageWidth; col++) {
//            QRgb rgbIm = processedImage->pixel(col, row);
            QRgb rgbIm = image.pixel(col, row);
            QRgb rgbBg = background.pixel(col, row);
...
//            int r = (double)qRed(rgbIm) * fmin(fmax(means[0] / ((double)qRed(rgbBg) + eps), minr), maxr) + means[0];
//            int g = (double)qGreen(rgbIm) * fmin(fmax(means[1] / ((double)qGreen(rgbBg) + eps), minr), maxr) + means[1];
//            int b = (double)qBlue(rgbIm) * fmin(fmax(means[2] / ((double)qBlue(rgbBg) + eps), minr), maxr) + means[2];
            processedImage->setPixel(col, row, qRgb(r,g,b));
        }
    }
